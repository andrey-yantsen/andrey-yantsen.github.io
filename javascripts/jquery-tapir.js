      ...
      $.getJSON(
        'http://tapirgo.com/api/1/search.json?token=' + settings.token + '&query=' + paramValue(settings.query_param) + '&callback=?', function(data){
          ...
          document.getElementById('search_results').style.overflow="hidden";
            ...
            el.append('<div id="content" class="inner"><article class="post"><h2 class="title"><a href="' + val.link + '">' + val.title + '</a></h2><div class="entry-content"><div class="meta"><div class="date">Опубликовано: ' + (val.published_on).substr(0,10) + '</div></div><p>' + str2 + '...</p><a href="' + val.link +'" class="more-link">Читать дальше</a></div></article></div>');
            if(key >= (data.length-1)){
                el.append('<div id="content" class="inner"><center><h2 style="border-bottom: 1px solid #ddd;">Конец</h2></center><br><br></div>');
            }
          });
        }
      );
